...
namespace PofudukFilo.Tests
{
    ...
    {
        // Expected values: the table in design/gdd/game-concept.md §5 × XpScale 3.25 (threat.md §3.6).
        [TestCase(1, 64)]
        [TestCase(2, 100)]
        [TestCase(3, 139)]
        [TestCase(5, 223)]
        ...
        {
            Assert.That(Formulas.SpawnBudget(0f, 10f), Is.EqualTo(3.5f * 1.15f).Within(1e-5f));
            Assert.That(Formulas.SpawnBudget(0f, 0f), Is.EqualTo(3.5f * 0.75f).Within(1e-5f));
        }
        ...
        public void test_power_deficit_is_one_with_enough_power_and_capped()
        {
            ...
            Assert.That(Formulas.DeficitHpScale(1.25f), Is.EqualTo(1.3668f).Within(1e-3f));
            Assert.That(Formulas.DeficitDamageScale(1.25f), Is.EqualTo(1.1433f).Within(1e-3f));
            // The ceiling stays beatable: at most HP ×2.64, damage ×1.52.
            Assert.That(Formulas.DeficitHpScale(Formulas.MaxPowerDeficit), Is.LessThan(2.7f));
            Assert.That(Formulas.DeficitDamageScale(Formulas.MaxPowerDeficit), Is.LessThan(1.55f));
        }
